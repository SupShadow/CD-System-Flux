...
import { getCountdownTrack, FALLBACK_STREAMING_LINKS, type Track } from "@/lib/tracks";

// Streaming platform icon definitions
...
// Helper function to get streaming links for a track (uses track-specific or fallback)
function getStreamingLinks(track: Track | undefined) {
    return STREAMING_PLATFORMS.map(platform => ({
        ...platform,
        url: track?.streamingLinks?.[platform.key] || FALLBACK_STREAMING_LINKS[platform.key]
    }));
}
        ...
        <section className="w-full max-w-3xl mx-auto">
            ...
            >
                ...
                {isReleased && (
                    ...
                    >
                        {getStreamingLinks(featuredTrack).map((link, index) => (
                            <motion.a
                                key={link.name}
                                href={link.url}
                                target="_blank"
